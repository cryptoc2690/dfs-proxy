...
def build_candidates(players, n, *, teams, split_targets=None, rng=None,
    ...
    while len(out) < n and tries < n * 40:
        ...
        # A forced player is seeded the way a core is: dropped in before the
        # fill, then the ordinary construction finishes the roster under every
        # ordinary rule — split shape, team counts, salary, the pool. It is a
        # carve-out in WHO is in the lineup, never in how the lineup is built.
        if force is not None and force.dk_id != cpt.dk_id:
            if (left.get(force.team, 0) <= 0
                    or salary + force.salary > SALARY_CAP):
                continue
            picked.append(force)
            used.add(force.dk_id)
            left[force.team] -= 1
            salary += force.salary
            if not (force.in_pool or force.core):
                off += 1
        ok = True
        for _ in range(ROSTER_SIZE - len(picked)):
...
def lottery_targets(final, pool_players, k=LOTTERY_TICKETS):
    ...
    projection, so a player priced above his projection's tier is drawn into
    none of four thousand candidates and appears in exactly zero of 900 roster
    spots. On ATL @ GB that was Austin Hooper — $1,600 for 1.11, above the $200
    and $600 salary punts and below the $2,000 and $3,000 real ones, so no
    lineup in the search ever wanted him — and the winning roster had him.
    MarShawn Lloyd went zero the same way: Kaleb Johnson projects 7.02 at
    $4,000 against his 7.03 at $7,400. Exposure tracks distance from the
    price/projection frontier and past some distance it snaps to exactly zero,
    so more entries only multiply the same answer.

    ONE player per ticket. Two punts in one roster is not a lottery ticket, it
    is a lineup that needs two miracles, and it hides one missed player behind
    another. With more missed players than tickets the expensive ones go first
    and the rest are NAMED: a $200 body projecting 0.44 cannot win a slate, a
    $7,400 back squeezed out on price can. Knowing who was left over is the
    useful half — take them off the sheet and the next build spends those
    tickets on somebody who can do something.

    Ships on judgement, not measurement. The cost is small and the benefit is a
    tail event no archive we will ever hold can separate from noise. The
    argument is structural: 70% of showdown lineups carry a dupe, so being
    different is the only edge on offer; a symmetric standard deviation cannot
    represent an outcome that is bimodal — near zero, or a touchdown, and
    Hooper's SD of 2.18 around a mean of 1.11 makes his realistic scoring game a
...
def select(lineups, n, *, captain_cap=None,
    ...
    #
    # The cheap slot is a CLUSTER, not a hierarchy, and the tool's score cannot
    # resolve it. That part is solid and worth keeping: on DET @ BUF the lineups
    # ranked fifth and eighth differed in one slot (Bills defence against Joshua
    # Palmer) and their scores were 0.0003 apart against a standard error of
    # 0.0022 — a seventh of one error bar — yet the defence took 33 of 150 and
    # Palmer none. Asked directly, the same simulator has Palmer beating that
    # defence 27% of the time, Knox 33%, Coleman 35%, and Gore, who took 35 of
    # the 150, 21%.
    #
    # Acting on it still failed. select() was made to band candidates within
    # TIE_SE standard errors and order inside a band by lowest summed ownership.
    # Over the same four showdowns, four seeds, realised money: -$30 at half an
    # error (helped 0 of 4), -$66 at one (0 of 4), -$52 at two, -$159 at three,
    # with cash falling monotonically -10, -17, -33, -59. It did what it was
    # built to do — Palmer went from 2 of 150 to 20, Knox 4 to 13, Coleman 0 to
    # 15, and DET @ BUF improved from $13 to $23 with its best lineup moving from
    # rank 15,882 to 1,703 — and it lost money on the three slates it was not
    # derived from.
    #
    # The likely reason is worth writing down: "break ties toward lower
    # ownership" is a FADE wearing the word coverage, applied exactly where the
    # tool knows least. Fading is already measured as negative here. If the band
    # idea is revisited, the tiebreak has to be something other than ownership.
    #
