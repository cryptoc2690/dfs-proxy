...
// DVP — forward to Vercel (manually updated)
...
app.listen(PORT, () => console.log(`Proxy running on port ${PORT}`));      if (g.home_team?.abbreviation) todayTeams.add(g.home_team.abbreviation);
      if (g.visitor_team?.abbreviation) todayTeams.add(g.visitor_team.abbreviation);
    }
    const yestTeams = new Set();
    for (const g of yestData.data || []) {
