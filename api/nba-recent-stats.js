export default async function handler(req, res) {
  ...
  try {
    ...
    // Step 2: fetch advanced stats + regular stats in parallel, chunked
    const chunks = [];
    ...
    // Step 3: build minutes map from regular stats keyed by player+game
    const minutesMap = {};
    for (const s of regularStats) {
      const name = `${s.player?.first_name} ${s.player?.last_name}`.trim();
      const gameId = s.game?.id;
      if (name && gameId) minutesMap[`${name}::${gameId}`] = s.min || 0;
    }

    // Step 4: group advanced stats by player, last 5 games
    const byPlayer = {};
    for (const s of advancedStats) {
      if (s.period !== 0) continue; // period 0 = full game totals only
      const name = `${s.player?.first_name} ${s.player?.last_name}`.trim();
      ...
      byPlayer[name].push({
        ...
        pace: s.pace || 0,
        touches: s.touches || 0,
        possessions: s.possessions || 0,
      });
    }

    // Step 5: compute trends per player
    const players = Object.entries(byPlayer).map(([name, games]) => {
      ...
      // Usage spike: recent usage significantly above 5-game avg
      const usageSpike = recentUsage > avgUsage + 4;
      // Minutes risk: recent minutes significantly below avg
